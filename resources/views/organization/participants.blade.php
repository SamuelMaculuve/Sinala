...
<section class="mt-5 overflow-hidden rounded-3xl bg-white"><div class="overflow-x-auto"><table class="w-full min-w-[720px] text-left"><caption class="sr-only">Participantes filtrados</caption><thead><tr><th>Nome</th><th>Contacto</th><th>Organização</th><th>Eventos</th><th></th></tr></thead><tbody>@forelse($participants as $participant)<tr><td class="font-semibold">{{ $participant->full_name }}</td><td>{{ $participant->phone ?: $participant->email ?: '—' }}</td><td>{{ $participant->company ?: '—' }}</td><td>{{ $participant->events_count }}</td><td class="text-right"><a class="btn-secondary min-h-10 px-4 text-sm" href="{{ route('participants.edit',$participant) }}">Editar</a></td></tr>@empty<tr><td colspan="5" class="py-12 text-center text-stone-400">Nenhum participante corresponde aos filtros.</td></tr>@endforelse</tbody></table></div></section>
